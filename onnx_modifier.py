...
class onnxModifier:
    ...
    def gen_name2module_map(self):
        ...
        for out in self.graph.output:
            self.node_name2module[
                "out_" + out.name] = out  # add `out_` in case the output has the same name with the last node
    ...
    def add_output(self, output):
        ...
        for idx, node in enumerate(shape_info.graph.value_info):
            if node.name in output.values():
                value_info_protos.append(node)

        model_proto = copy.deepcopy(self.model_proto)
        model_proto.graph.output.extend(value_info_protos)
        return model_proto
